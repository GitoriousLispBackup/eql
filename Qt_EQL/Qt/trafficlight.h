...
public:
    ...
    QState *createLightState(LightWidget *light, int duration, QState *parent = 0)
    {
        ...
        QObject::connect(timing, QSIGNAL(entered()), light, QSLOT(turnOn()));
        QObject::connect(timing, QSIGNAL(entered()), timer, QSLOT(start()));
        QObject::connect(timing, QSIGNAL(exited()), light, QSLOT(turnOff()));
        QFinalState *done = new QFinalState(lightState);
        timing->addTransition(timer, QSIGNAL(timeout()), done);
