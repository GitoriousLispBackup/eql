...
(load "3rd-party/sokoban")
(load "3rd-party/levels")
...
(defun create-view ()
  (setf *scene* (qnew "QGraphicsScene"
		      ...
		      "backgroundBrush" (qnew "QBrush(QColor)" "#666666"))
  ...
  (flet ((add (item type)
	...
	(unless (char= #\Space char)
	  (let* ((type (char-type char))
	    ...
	    (add item type)))))
    (add (create-item :player2) :player2)
    (dotimes (i (length (cdr (assoc :object *items*))))
      (add (create-item :object2) :object2))))

(defun create-item (type)
  (let* ((char (type-char type))
	 (file (format nil "pics/~(~a~).png" type))
	 (item (if (probe-file file)
		   (qnew "QGraphicsPixmapItem(QPixmap)" (qnew "QPixmap(QString)" file))
		   (let ((item (qnew "QGraphicsTextItem")))
		     (qfun item "setHtml"
...
(defun draw-items (type)
  (let* ((char (type-char type))
	 ...
	 (y 0)
	 (curr (assoc* type *items*)))
    (flet ((hide ()
	     (unless (eql :wall type)
	       (dolist (item curr)
		 (qfun* (first curr) "QGraphicsItem" "setVisible" nil)))))
      (hide)
      (dolist (row (maze-text *maze*))
	(let ((x 0))
	  (do-string (curr-char row)
	    (when (char= char curr-char)
	      (do- (qfun* (first curr) "QGraphicsItem")
		("setPos" (list x y))
		("setVisible" t))
	      (setf curr (rest curr)))
