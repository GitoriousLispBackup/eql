...
(defparameter *clock* (qnew "QWidget" "size" (list 170 170) "pos" (list 50 50)))

(defun start ()
  (let ((timer (qnew "QTimer")))
    (qconnect timer "timeout()" #'(lambda () (qfun *clock* "update()")))
    (qoverride *clock* "paintEvent(QPaintEvent*)" 'paint)
    (qfun timer "start(int)" 500)
    (qfun *clock* "show()")))

(defun pen (width &optional (color "black"))
  (let ((pen (qnew "QPen")))
    (qfun pen "setCapStyle(Qt::PenCapStyle)" "RoundCap")
    (qfun pen "setWidth(int)" width)
    (qfun pen "setColor(QColor)" color)
    pen))
