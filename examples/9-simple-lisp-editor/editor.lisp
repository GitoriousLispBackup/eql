      ...
      (let ((cursor (qfun *current-editor* "textCursor"))
        ...
        (when copy
          (dotimes (n #+darwin 25 #-darwin 1) ; hack
            (qfun *current-editor* "repaint")
            (qprocess-events))
          (sleep 0.2))
        (qfun *current-editor* (if copy "copy" "cut"))
        (setf cursor (qfun *current-editor* "textCursor"))
        (qfun cursor "setPosition" pos-open)
        (qfun *current-editor* "setTextCursor" cursor)
        (qsingle-shot 100 (lambda () (qfun *current-editor* "setFocus"))))))) ; hack
