...
(defun start ()
  (let ((window       (qnew "QWidget"
    ...
    (dolist (w (list query-label query-edit))
      (qfun query-layout "addWidget(QWidget*)" w))
    (qfun main-layout "addLayout(QLayout*)" query-layout)
    (qfun main-layout "addWidget(QWidget*)" result-view)
    (qfun window "setLayout(QLayout*)" main-layout)
    (setup-model-and-view result-view)
    (qfun window "show()")))

(defun setup-model-and-view (view)
  (let ((model (qnew "QStandardItemModel")))
    (qfun model "setHorizontalHeaderLabels(QStringList)" (list "Name" "Office"))
      ...
      (let (items)
        (dolist (text row)
          (let ((item (qnew "QStandardItem")))
            (qfun item "setText(QString)" text)
            (push item items)))
        (qfun model "appendRow(QList<QStandardItem*>)" (nreverse items))))
    (qfun view "setModel(QAbstractItemModel*)" model)
    (qfun (qfun view "verticalHeader()") "hide()")
    (qfun (qfun view "horizontalHeader()") "setStretchLastSection(bool)" t)))
