  ...
  (macrolet ((with-save (() &body body)
    ...
    (qlet ((p "QPainter(QWidget*)" *clock*))
      (let* ((size (|size| *clock*))
        ...
        (|translate(QPointF)| p (mapcar (lambda (x) (/ x 2)) size))
        (|scale| p scale scale))
      (|rotate| p -90)
      (|setRenderHint| p |QPainter.Antialiasing|)
      (|setPen(QPen)| p *pen-clock*)
      (|setBrush(QBrush)|  p *brush-clock*)
      (|drawEllipse(QRect)| p '(-75 -75 150 150))
      (|setPen(QPen)| p *pen-hour-marks*)
      (dotimes (n 12)
        (|rotate| p 30)
        (|drawLine(QLine)| p '(55 0 65 0)))
      (|setPen(QPen)| p *pen-minute-marks*)
      (dotimes (n 60)
        (|rotate| p 6)
        (|drawLine(QLine)| p '(63 0 65 0)))
      (multiple-value-bind (sec min hour)
          (get-decoded-time)
        (with-save ()
          (|rotate| p (+ (* 30 hour) (/ min 2)))
          (|setPen(QPen)| p *pen-hour*)
          (|setOpacity| p 0.5)
          (|drawLine(QLine)| p '(-10 0 36 0)))
        (when *show-minutes*
          (with-save ()
            (|rotate| p (* 6 min))
            (|setPen(QPen)| p *pen-minute*)
            (|setOpacity| p 0.5)
            (|drawLine(QLine)| p '(-15 0 65 0))))
        (when *show-seconds*
          (with-save ()
            (|rotate| p (* 6 sec))
            (|setPen(QPen)| p *pen-second*)
            (|setBrush(QBrush)| p *brush-second*)
            (|drawEllipse(QRectF)| p '(-1.5 -1.5 3 3))
            (|setOpacity| p 0.7)
            (|drawLine(QLine)| p '(-15 0 52 0))
            (|drawEllipse(QRect)| p '(53 -4 8 8))))))))
